...
export async function loadAccountReadiness(): Promise<AccountReadiness | null> {
  const { data, error } = await supabaseBrowser().rpc('my_account_readiness');
...
export async function loadAccountPolicyVersions(): Promise<AccountPolicyVersions | null> {
  const { data, error } = await supabaseBrowser().rpc('current_account_policy_versions');
  if (error) throw error;
...
export async function verifyEmailCode(email: string, token: string): Promise<void> {
  ...
  if (error) throw error;
  const { error: proofError } = await supabaseBrowser().rpc('record_my_verified_email_otp');
  if (proofError) throw proofError;
}

export async function requestPhoneVerification(phone: string): Promise<void> {
  const { error } = await supabaseBrowser().auth.updateUser({ phone: phone.trim() });
  if (error) throw error;
}
...
export async function verifyPhoneCode(phone: string, token: string): Promise<void> {
  ...
  if (error) throw error;
  const { error: syncError } = await supabaseBrowser().rpc('sync_verified_auth_phone_to_private_contact');
...
export async function acceptCurrentAccountPolicies(consent: AccountPolicyConsent): Promise<void> {
  const { error } = await supabaseBrowser().rpc('accept_current_account_policies', {
