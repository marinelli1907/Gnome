...
This is the final database gate for the Gnome 1.1.0 release candidate. The SQL
has passed the Postgres 17 clean-room suites. It has **not** been applied to
production by the coding agent: production writes require the owner.
...
Run one complete file at a time in the Supabase production SQL editor. Each file
is transaction-wrapped. Stop immediately if a file reports an error; do not skip
ahead and do not rerun only a fragment of a failed file.
...
Run this read-only proof after all five files. Every column must return `true`.
...
After the live proof succeeds, move only these five rows from
`supabase/migrations/UNAPPLIED.txt` into `supabase/migrations/APPLIED.tsv`, using
the exact production statement hashes. Re-run:
