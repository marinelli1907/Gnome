#!/usr/bin/env bash
# Behavioural tests for the 0106 publish-allowance model, against a THROWAWAY local database.
#
...
# needed before 0106 can even be exercised:
#
#   profiles.suspended            — 0103's listings_block_suspended() trigger reads it
#   market_effective_plan(uuid)   — 0106's allowance trigger resolves the plan through it
#
...
# 0106 must still be intact after the backfill.
for fn in promo_validate listing_overage_required create_publish_authorization \
          mark_authorization_paid record_promo_redemption admin_promo_campaigns; do
  psql -h "$HOST" -d "$DB" -tAc "select to_regproc('public.${fn}')" | grep -q . \
    || { echo "MISSING after build: ${fn}" >&2; exit 1; }
done

psql -h "$HOST" -d "$DB" -v ON_ERROR_STOP=1 -f "$here/promotions_suite.sql"
