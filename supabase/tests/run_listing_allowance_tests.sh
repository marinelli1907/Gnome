#!/usr/bin/env bash
# Behavioural tests for the 0104 publish-allowance model, against a THROWAWAY local database.
#
...
# needed before 0104 can even be exercised:
#
#   profiles.suspended            — 0103's listings_block_suspended() trigger reads it
#   market_effective_plan(uuid)   — 0104's allowance trigger resolves the plan through it
#
...
# 0104 must still be intact after the backfill.
for fn in enforce_publish_allowance listing_type_spends_allowance market_allowance_period \
          market_allowance_usage my_listing_allowance renew_listing; do
  psql -h "$HOST" -d "$DB" -tAc "select to_regproc('public.${fn}')" | grep -q . \
    || { echo "MISSING after build: ${fn}" >&2; exit 1; }
done

psql -h "$HOST" -d "$DB" -v ON_ERROR_STOP=1 -f "$here/listing_allowance_suite.sql"
